...
from accounts.models import Subjects , SessionYearModel , Students , Attendance , AttendanceReport , LeaveReportStaff , \
    Staffs , FeedBackStaff , CustomUser , Courses , StudentResult
...
def get_students(request):
    ...
    for student in students:
        data_small = {"id": student.admin.id, "roll_number": student.roll_number, "name": student.admin.first_name+" "+student.admin.last_name}
...
def get_attendance_student(request):
    ...
    for student in attendance_data:
        data_small = {"id":student.student_id.admin.id, "roll_number": student.student_id.roll_number, "name":student.student_id.admin.first_name+" "+student.student_id.admin.last_name,"status":student.status}
        list_data.append(data_small)
    return JsonResponse(json.dumps(list_data), content_type="application/json", safe=False)
...
def save_student_result(request):
    ...
    final_term=request.POST.get('final_term')
    sessional_marks = request.POST.get ('sessional_marks')
    subject_id=request.POST.get('subject')
    ...
    try:
        ...
        if check_exist:
            ...
            result=StudentResult(student_id=student_obj,subject_id=subject_obj,subject_mid_term=mid_term,subject_final_term=final_term, subject_sessional_marks=sessional_marks)
            result.save()
...
def fetch_result_student(request):
    ...
    if result:
        ...
        result_data={"mid_term":result.subject_mid_term,"final_term":result.subject_final_term, "sessional_marks":result.subject_sessional_marks}
