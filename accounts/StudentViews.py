...
from accounts.models import Students, Courses, Subjects, CustomUser, Attendance, AttendanceReport, FeedBackStudent, \
    LeaveReportStudent, SessionYearModel, StudentResult
...
def student_view_result(request):
    ...
    for row in studentresult:
        mid = float (row.subject_mid_term)
        final = float (row.subject_final_term)
        sessional = float (row.subject_sessional_marks)
        total = mid + final + sessional
        row.total = total
        datalist.append (row)


    return render (request , "student_template/student_result.html" , {"studentresult": datalist})